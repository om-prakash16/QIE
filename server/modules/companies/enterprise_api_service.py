...
class EnterpriseApiService:
    ...
    def generate_api_key(self, company_id: str, label: str, scopes: List[str] = ["read.proof_score", "read.skills"]) -> str:
        """
    ...
    async def validate_api_key(self, api_key: str) -> Optional[Dict[str, Any]]:
        ...
        resp = db.table("enterprise_api_keys") \
        ...
        if resp.data:
            ...
            db.table("enterprise_api_keys").update({"last_used_at": "now()"}).eq("api_key_hash", key_hash).execute()
            return resp.data
    ...
    def revoke_api_key(self, key_id: str):
        ...
        db = get_supabase()
        if db:
            db.table("enterprise_api_keys").update({"is_active": False}).eq("id", key_id).execute()
