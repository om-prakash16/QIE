...
class CareerRiskService:
    ...
    async def get_career_risk_analysis(self, user_id: str, profile_data: Dict[str, Any], proof_score: float) -> Dict[str, Any]:
        """
        ...
        salary_gap = profile_data.get("salary_market_gap_percent", random.uniform(-10, 20))
        skill_velocity = profile_data.get("skill_acquisition_rate", random.uniform(0.5, 5.0))
        
        job_change = self._calculate_job_change_probability(tenure_months, salary_gap, proof_score)
        growth = self._predict_growth_trajectory(skill_velocity, proof_score, profile_data.get("experience_years", 3))
        stagnation = self._assess_skill_stagnation(profile_data.get("last_skill_update_days", 90), profile_data.get("skills", []), profile_data.get("role", "Software Engineer"))
    ...
    def _calculate_job_change_probability(self, tenure_months: int, salary_gap: float, proof_score: float) -> Dict[str, Any]:
        """
        ...
        score_modifier = (proof_score / 1000) * 0.2 # Higher score = more recruiter attention
        
        final_prob = min(0.99, max(0.01, base_prob + salary_modifier + score_modifier))
        
        return {
            "score": round(final_prob * 100, 1),
            "level": "High" if final_prob > 0.7 else "Medium" if final_prob > 0.4 else "Low",
            "features": [
                {
                    "name": "Tenure Peak",
                    "impact": "High",
    ...
    def _predict_growth_trajectory(self, skill_velocity: float, proof_score: float, exp_years: int) -> Dict[str, Any]:
        """
        ...
        level = "Accelerated" if growth_index > 0.75 else "Steady" if growth_index > 0.4 else "At Risk"
        
        return {
            "score": round(growth_index * 100, 1),
            "level": level,
            "next_milestone_estimate": "12-18 months" if growth_index > 0.6 else "24+ months",
            "features": [
                {
    ...
    def _assess_skill_stagnation(self, days_since_update: int, skills: List[str], role: str) -> Dict[str, Any]:
        """
        ...
        if any(s.lower() in ["solidity", "rust", "react", "next.js", "llm"] for s in skills):
        ...
        return {
            ...
            "level": "Critical" if final_risk > 0.8 else "Warning" if final_risk > 0.5 else "Healthy",
            "features": [
                {
    ...
    def _generate_summary(self, job_change, growth, stagnation) -> str:
        ...
        return "Your career is on a steady upward trajectory with balanced risk metrics."
