...
@router.post("/build")
async def extract_and_build_portfolio(req: PortfolioBuildRequest, current_user = Depends(get_current_user)):
    """
    ...
    proof_data = rep_resp.data[0] if rep_resp.data else {"total_score": 0, "skills_score": 0, "github_score": 0}
    ...
    skills = user_data.get("profile_data", {}).get("skills", ["Rust", "Python", "Solana"])
    github_handle = user_data.get("dynamic_fields", {}).get("github_handle", "developer")
